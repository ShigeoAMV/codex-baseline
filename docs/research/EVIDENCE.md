...
The official OpenAI manual was refreshed on 2026-08-13 with the bundled
...
| Native Windows | Windows PowerShell 5.1 reachable through WSL interop; native Windows `codex` not installed | PowerShell scripts can be executed, but native Codex integration must be labelled unverified unless installed later |
...
| Subagents | Current local clients support subagents and custom project/user agent TOML. Subagents cost more tokens; read-heavy bounded exploration/test/triage is the recommended starting point. Writable parallelism can conflict. | Encode selection criteria, not unconditional fan-out. Use fresh read-only reviewers for substantial work and worktrees only for genuinely independent writes. | [Subagents](https://learn.chatgpt.com/docs/agent-configuration/subagents) |
