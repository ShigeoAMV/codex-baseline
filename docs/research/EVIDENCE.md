...
| GPT-5.6 bounded work | The guide identifies Sol as the flagship GPT-5.6 route and describes GPT-5.6 as proactive and persistent on multi-step work. It recommends lean prompts, one compact autonomy policy, explicit stop/retry limits, and no repetition of completed work. Checked 2026-08-20. | Add model-neutral scope and convergence invariants to the small global block; do not add a Sol mode, force a lower reasoning level, or claim the policy eliminates loops. | [Latest model guide](https://developers.openai.com/api/docs/guides/latest-model) |
...
| Non-interactive mode | `codex exec --json` emits JSONL tool/file/plan events and parent-turn usage, but its public event contract does not expose the complete Subagent graph. | Retain it for containment probes and ordinary automation, not as the sole orchestration-evidence source. | [Non-interactive mode](https://learn.chatgpt.com/docs/non-interactive-mode) |
| App Server telemetry | App Server 0.147 streams thread lifecycle/status, collaboration tool calls (including requested model/effort and receiver thread IDs), settings/reroutes, turn/item timestamps, and per-thread token updates over JSONL stdio. Generated 0.147 schemas also expose parent thread IDs and spawn depth. | Use a pinned stdio runner plus a closed reducer for benchmark orchestration evidence. Keep write/test isolation, dollar cost, and external attestation separate; never infer them from model events. | [App Server](https://learn.chatgpt.com/docs/app-server), local `codex app-server generate-json-schema --experimental` probe, 2026-08-16 |
...
- App Server exposes the missing core orchestration telemetry, but benchmark fairness still
